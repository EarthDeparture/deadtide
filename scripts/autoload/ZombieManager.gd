...
func register_spawn_point(spawn_point: Node3D):
    ...
    print("Registered spawn point: ", spawn_point.name)

func spawn_wave(round_number: int):
    # Calculate zombie count: 6 + (4 * round)
    var zombie_count = 6 + (4 * round_number)
    print("Spawning wave of ", zombie_count, " zombies")

    for i in range(zombie_count):
        _spawn_zombie()
        # Stagger spawns for more organic feel
