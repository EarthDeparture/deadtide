...
var player_data: Dictionary = {}  # player_id -> { points, is_downed, has_revived }
...
func set_player_downed(player_id: int, downed: bool):
    if player_id in player_data:
        ...
        if downed:
            EventBus.player_downed.rpc_id(player_id, player_id)
...
func check_game_over():
    ...
    for player in players:
        var player_id = player.get_instance_id()
        if player_id in player_data and not player_data[player_id]["is_downed"]:
            all_downed = false
            break
